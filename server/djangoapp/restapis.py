...
#                 # dealer_obj = CarDealer(address=dlr_data["address"], city=dlr_data["city"], full_name=dlr_data["full_name"],
#                 #                     id=dlr_data["id"], lat=dlr_data["lat"], long=dlr_data["long"],
#                 #                     short_name=dlr_data["short_name"], 
#                 #                     st=dlr_data["st"], state=dlr_data["state"], zip=dlr_data["zip"])
...
def analyze_review_sentiments(text):
    url = "https://api.us-south.natural-language-understanding.watson.cloud.ibm.com/instances/c0f6671a-2162-4a8b-a52c-a6a62389fe57"
    api_key = "h01PCL3Jz_06_Gqh4sWOJYxL38XcVKW--GCC9YXalsHn"
    authenticator = IAMAuthenticator(api_key)
    natural_language_understanding = NaturalLanguageUnderstandingV1(version='2021-08-01',authenticator=authenticator)
    natural_language_understanding.set_service_url(url)
    response = natural_language_understanding.analyze( text=text+"hello hello hello",features=Features(sentiment=SentimentOptions(targets=[text+"hello hello hello"]))).get_result()
